...
// Dados fictícios para visualização - Apenas Ações
const mockStocks: Investment[] = [
  {
    id: '1',
    symbol: 'PETR4',
    name: 'Petrobras PN',
...
const allocationBySector = [
  { name: 'Energia', percentage: 12.9, color: 'bg-gray-900' },
  { name: 'Mineração', percentage: 16.5, color: 'bg-gray-700' },
  { name: 'Financeiro', percentage: 13.8, color: 'bg-gray-500' },
  { name: 'Consumo', percentage: 6.3, color: 'bg-gray-400' },
  { name: 'Industrial', percentage: 8.8, color: 'bg-gray-300' },
];
...
export default function StocksPage() {
  ...
  // Calcular totais
  const totalValue = stocks.reduce((sum, inv) => sum + inv.totalValue, 0);
  const totalCost = stocks.reduce((sum, inv) => sum + inv.totalCost, 0);
  const totalGainLoss = stocks.reduce((sum, inv) => sum + inv.gainLoss, 0);
                    ...
                    <TableCell>
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 bg-gray-100 rounded-full flex items-center justify-center flex-shrink-0">
                          <span className="text-xs font-semibold text-gray-700">{stock.symbol}</span>
                        </div>
                        <div>
