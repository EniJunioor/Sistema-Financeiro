  ...
  try {
    ...
    if (!response.ok) {
      ...
      try {
        ...
        const msg = errorJson.error || errorJson.message || errorText;
        // NestJS ValidationPipe retorna message como string ou string[]
    ...
    if (error instanceof Error) {
      ...
      if (isNetworkError) {
        throw new Error("Não foi possível conectar ao servidor. Verifique sua internet e se o backend está rodando.");
      }
      throw error;
    }
...
// Email/password login (NestJS backend)
